...
> **Observation (useful in implementation).** For a position $t=(r,c)$, the equality (B1) splits naturally into horizontal and vertical flows:
...
- For each number $t$ with candidate literal set $A_t$, maintain lower/upper bounds on each literal ($0/1$ if already fixed). Let $$ \begin{align} L_t &= \sum_{x \in A_t} \mathrm{lb}(x) \quad \text{and} \\ U_t &= \sum_{x \in A_t} \mathrm{ub}(x) \end{align}$$
